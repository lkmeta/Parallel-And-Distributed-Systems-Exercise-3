...
%
  ...
  patchCube = @(X,w) ...
      permute( ...  % B = permute(A,dimorder) rearranges the dimensions of an array in the order specified by the vector dimorder. For example, permute(A,[2 1]) switches the row and column dimensions of a matrix A.
          reshape( ...
              im2col( ...   % B = im2col(A,[m n],'sliding') rearranges sliding image neighborhoods of size m-by-n into columns with no zero-padding, and returns the concatenated columns in matrix B
                  padarray( ...
                      X, ...
                      (w-1)./2, 'symmetric'), ...
                  w, 'sliding' ), ...
